...
public partial class HomeRpdPlaylistView : ContentView
{
    ...
    private RpdSettings? RpdSettings { get; set; }
    private bool _isInitialized = false;

    public HomeRpdPlaylistView(RpdSettings rpdSettings)
    {
        InitializeComponent();

        RpdSettings = rpdSettings;
    ...
    internal async Task Init()
    {
        ...
        try
        {
            ...
            // Apply filter last.
            var songParts = RefreshRpdSizeLabels();
            progress++;

            HandleAutoStartRpd(songParts);
    ...
    internal void HomeModeSegmentedControlSelectionChanged(object? sender, Syncfusion.Maui.Buttons.SelectionChangedEventArgs? e)
    {
        RpdSettings.UsingGeneratePlaylist = (HomeModeSegmentedControl.SelectedIndex == 1);
        UpdateModeControls();
    }

    internal void UpdateModeControls()
    {
        HomeModeSegmentedControl.SelectionChanged -= HomeModeSegmentedControlSelectionChanged;

        HomeModeSegmentedControl.SelectedIndex = RpdSettings.UsingGeneratePlaylist ? 1 : 0;
        ...
        ParentPage!.Title = RpdSettings.UsingGeneratePlaylist ? "Generate playlist" : "Start RPD";
    ...
    private void UpdateDuration(bool loadValue = false)
    {
        if (loadValue)
        {
            ...
            DebugService.Instance.Debug($"HomeRpdPlayerView: Duration: {hours}");
    ...
    private List<SongPart> RefreshRpdSizeLabels()
    {
    ...
    internal void HandleAutoStartRpd(List<SongPart> songParts)
    {
        if (Preferences.ContainsKey(CommonSettings.START_RPD_AUTOMATIC))
        {
            ...
            if (startRpd) { StartRpd(songParts); }
        }
    }


    private void StartRpd(List<SongPart> songParts)
    {
        ...
        RpdSettings.PlayRandomSong(songParts);
    }
    ...
    {
        Preferences.Set(CommonSettings.HOME_DURATION, RpdSettings.Duration.TotalHours);
